...
export default function MoviesPage({ genres: rawGenres = [], onSelectMovie, onPlayMovie }: { genres?: any[], onSelectMovie?: (id:number, type?:'movie'|'tv')=>void, onPlayMovie?: (id:number|string, type?:'movie'|'tv', params?:Record<string,any>)=>void }) {
  ...
  useEffect(() => {
    // fetch a featured movie for the hero (random from top 10 with available YouTube video)
    (async () => {
      try {
        ...
        <section key={g.id} style={{marginBottom:20}}>
          {loadingMap[g.id] ? (
            ...
            (() => {
              ...
              // Normalize movie items to include `title` and `media_type` so `Row` can fetch logos
              const normalized = list.slice(0,12).map((m:any) => ({ ...m, title: m.title || m.name, media_type: 'movie' }));
              return (
    ...
    <Box pt="200px">
      ...
      <div ref={containerRef} style={{height:'70vh', overflow:'auto', overscrollBehavior: 'contain'}}>
        <div className="movie-grid genre-grid" style={{gridTemplateColumns:'repeat(auto-fill, minmax(160px, 1fr))', paddingBottom:20}}>
          {items.map(i => (
            <GenreGridCard key={i.id} item={i} mediaType="movie" onSelect={onSelectMovie} onPlay={onPlayMovie} />
          ))}
        </div>
